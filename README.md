...
Command                                    | Action
:----------------------------------------- | :-----
`add -name {name} {polynomial expression}` | The name paramenter is optional. If not provided, the app should name the expression starting with `p1`, `p2` and so on, without repetition. **Examples**:<br>· `add -name poli 3x^3+2` – adds a polynomial `3x^3+2` named `poli`;<br>· `add 4x^2-3x` – adds the polynomial `4x^2-3x` named `p1`.
