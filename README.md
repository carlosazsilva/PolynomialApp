...
There are some strategies to represent and handle polynomial expressions, being the most common the use of arrays to store the different coefficients of the polynomial. The previous polynomial expressions could have the following computational representation using arrays:
...
Command                                    | Action
:----------------------------------------- | :-----
`add -name {name} {polynomial expression}` | The name paramenter is optional. If not provided, the app should name the expression by default, starting with `p1`, `p2` and so on, without repetition. **Examples**:<br>· `add -name poli 3x^3+2` – adds q polynomial `3x^3+2` named `poli`;<br>`add 4x^2-3x` – adds the polynomial `4x^2-3x` named `p1`.
